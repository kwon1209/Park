...
public class BoardDAO {
	...
	public List selectAllArticles(Map pagingMap){
		...
		try{
		   ...
		   String query = "SELECT * FROM ("
                   ...
                   + "  FROM ("
                   + "    SELECT @rownum:=@rownum+1 AS ROWNUM, LVL, articleNO, parentNO, title, id, writeDate"
	...
	public List selectAllArticles() {
		...
		try {
			...
			String query = "SELECT LEVEL,articleNO,parentNO,title,content,id,writeDate" + " from t_board"
					+ " START WITH  parentNO=0" + " WITH RECURSIVE articleNO=parentNO"
					+ " WITH RECURSIVE BY articleNO DESC";
			System.out.println(query);
			pstmt = conn.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public void deleteArticle(int articleNO) {
		try {
			...
			query += " WITH RECURSIVE  articleNO = parentNO )";
			System.out.println(query);
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, articleNO);
			pstmt.executeUpdate();
